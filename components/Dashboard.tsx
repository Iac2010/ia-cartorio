...
export const CostCenterDashboard: React.FC<CostCenterProps> = ({ financialData, onAddCostEntry, onDeleteCostEntry, companyLogo }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            const tableRows = financialData.costEntries.map(entry => `
                <tr>
                    <td>${entry.date.split('-').reverse().join('/')}</td>
                    <td>${entry.description}</td>
                    <td>${entry.category}</td>
                    <td style="text-align: right;">R$ ${entry.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                </tr>
            `).join('');
            ...
            printWindow.document.write(`
                ...
                <head>
                    ...
                    <style>
                        ...
                        h1, h2 { color: #1e293b; }
                        .summary { border: 1px solid #eee; padding: 15px; margin-bottom: 20px; background-color: #f9f9f9; border-radius: 8px; }
                ...
                <body>
                    ...
                    <div class="summary">
                        ...
                        <p><strong>Orçamento Total:</strong> R$ ${financialData.budget.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        <p><strong>Custo Total:</strong> R$ ${totalCost.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        <p><strong>Saldo Restante:</strong> R$ ${budgetStatus.remaining.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                    </div>
